              ...
              <div key={s.id} className={`tpl-section-card ${isOpen ? 'open' : ''}`}>
                ...
                >
                  ...
                  <div className="tpl-section-actions">
                    <button onClick={(e) => { e.stopPropagation(); moveSection(s.id, -1) }} title="Выше" aria-label="Переместить секцию выше">▲</button>
                    <button onClick={(e) => { e.stopPropagation(); moveSection(s.id, 1) }} title="Ниже" aria-label="Переместить секцию ниже">▼</button>
                    <button onClick={(e) => { e.stopPropagation(); duplicateSection(s.id) }} title="Дублировать" aria-label="Дублировать секцию">⎘</button>
                    <button onClick={(e) => { e.stopPropagation(); removeSection(s.id) }} title="Удалить" aria-label="Удалить секцию">×</button>
                  </div>
      ...
      {items.map((it, i) => (
        <div key={i} className="tpl-repeater-row">
          ...
          <div className="tpl-repeater-actions">
            <button type="button" onClick={() => move(i, -1)} title="Выше" aria-label="Выше">▲</button>
            <button type="button" onClick={() => move(i, 1)} title="Ниже" aria-label="Ниже">▼</button>
            <button type="button" onClick={() => remove(i)} title="Удалить" aria-label="Удалить элемент">×</button>
          </div>
        </div>
      ))}
