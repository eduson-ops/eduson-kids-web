  ...
  return (
    <>
      ...
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        title="Меню спавна (Q)"
        aria-label={open ? 'Закрыть меню спавна' : 'Открыть меню спавна (Q)'}
        aria-expanded={open}
      >
      ...
      {tool && !open && (
        <div className="spawn-placement-hint tool-hint">
          ...
          <button
            ...
            aria-label="Выключить инструмент"
          >
            ×
          </button>
        </div>
        ...
        <>
          ...
          <div ref={menuRef} className="spawn-menu" role="dialog" aria-modal="true" tabIndex={-1} aria-label="Добавить объект в сцену" onClick={(e) => e.stopPropagation()}>
            <header className="spawn-menu-head">
              ...
              <button className="spawn-menu-close" onClick={() => setOpen(false)} aria-label="Закрыть меню (Esc)">
                × <kbd>Esc</kbd>
              </button>
            </header>
            ...
            <div className="spawn-menu-body">
              ...
              <nav className="spawn-menu-sidebar">
                <button
                  ...
                  aria-pressed={activeCat === 'favs' && !query}
                >
                  <span aria-hidden>⭐</span><span>Избранное</span>
                ...
                <button
                  ...
                  aria-pressed={activeCat === 'recent' && !query}
                >
                  <span aria-hidden>🕘</span><span>Недавние</span>
                ...
                {CATALOG.map((c) => (
                  <button
                    ...
                    aria-pressed={activeCat === c.id && !query}
                  >
