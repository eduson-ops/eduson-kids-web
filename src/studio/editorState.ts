...
function persist() {
  ...
  saveTimer = window.setTimeout(() => {
    try {
      localStorage.setItem(STUDIO_KEY, JSON.stringify(state))
    } catch {
      /* quota exceeded */
    }
  }, 300)
...
export function updatePart(id: string, patch: Partial<PartObject>) {
  ...
  for (const k of Object.keys(patch) as Array<keyof PartObject>) {
    // PartObject — это discriminated union; через unknown получаем
    // generic-сейф каст без warning'а о несоответствии Record-типов.
    ;(beforePatch as Record<string, unknown>)[k] = (before as unknown as Record<string, unknown>)[k]
    ;(afterPatch as Record<string, unknown>)[k] = (patch as unknown as Record<string, unknown>)[k]
  }
