(function(){let e=`https://cdn.jsdelivr.net/pyodide/v0.26.2/full/`,t=null,n=null;async function r(){return t||n||(n=(async()=>{let n=await(await import(`${e}pyodide.mjs`)).loadPyodide({indexURL:e});return n.runPython(`
_commands = []
...
_reset()
`),t=n,n})(),n)}self.addEventListener(`message`,async e=>{let t=e.data;try{if(t.type===`ping`){await r(),self.postMessage({id:t.id,type:`ready`});return}if(t.type===`reset`){(await r()).runPython(`_reset()`),self.postMessage({id:t.id,type:`result`,commands:[]});return}if(t.type===`run`){let e=await r();e.runPython(`_reset()`),e.runPython(t.code);let n=e.globals.get(`_commands`).toJs({dict_converter:Object.fromEntries});self.postMessage({id:t.id,type:`result`,commands:n});return}}catch(e){let n=e instanceof Error?e.message:String(e);self.postMessage({id:t.id,type:`error`,message:n})}}),self.postMessage({id:0,type:`ready`}),r().catch(e=>{let t=e instanceof Error?e.message:String(e);self.postMessage({id:0,type:`error`,message:t})})})();