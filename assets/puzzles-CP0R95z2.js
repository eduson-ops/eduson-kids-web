...
</xml>`,l=[{id:"path-1",trainerId:"path",n:1,title:"Первый шаг",prompt:"Дойди до цветка на 1 клетке впереди. Один блок «идти вперёд» — и готово!",hints:["Найди блок «идти вперёд» в категории «Движение».","Поставь его внутрь «при запуске».","Достаточно одного шага: move_forward(1)."],starterBlocks:e,starterPython:`move_forward(1)
...
`,check:{kind:"build-pattern",strictColor:!0,expectedBlocks:[...i([[0,0,0],[1,0,0],[2,0,0],[0,0,1],[1,0,1],[2,0,1],[0,0,2],[1,0,2],[2,0,2]],"blue"),...i([[0,1,0],[1,1,0],[0,1,1],[1,1,1]],"yellow"),...i([[0,2,0]],"red")]},maxBlocks:16,reward:{coins:25,xp:30}},{id:"tower-10",trainerId:"tower",n:10,title:"Башня-пирамида 5",prompt:"Построй пирамиду 5×5 → 3×3 → 1×1 синего цвета. Подсказка: сдвигай основание, чтобы каждый этаж был по центру.",hints:["Три слоя: y=0 → 5×5, y=1 → 3×3 (сдвиг +1), y=2 → 1×1 (сдвиг +2).","Двойной for для каждого этажа.",'Можешь написать функцию square(sz, ox, oy): for i,j in range(sz): place_block(i+ox, oy, j+ox, "blue").'],starterBlocks:e,starterPython:`def layer(size, y, off):
    for i in range(size):
        for j in range(size):
...
layer(3, 1, 1)
layer(1, 2, 2)
`,check:{kind:"build-pattern",strictColor:!0,expectedBlocks:[...i(Array.from({length:5},(r,n)=>Array.from({length:5},(t,o)=>[n,0,o])).flat(),"blue"),...i(Array.from({length:3},(r,n)=>Array.from({length:3},(t,o)=>[n+1,1,o+1])).flat(),"blue"),...i([[2,2,2]],"blue")]},maxBlocks:20,reward:{coins:30,xp:35}}],d=[{id:"loop-1",trainerId:"loop",n:1,title:"Первый print",prompt:"Напечатай строку «Привет!» в консоль.",hints:["Используй print в категории «Текст».","Аргумент — строка «Привет!».",'print("Привет!")'],starterBlocks:e,starterPython:`print("Привет!")
...
`,check:{kind:"output-match",expected:["средний"]},reward:{coins:22,xp:28}},{id:"if-10",trainerId:"if",n:10,title:"FizzBuzz полный",prompt:"Для чисел 1..15 напечатай: «FizzBuzz» если делится на 15, «Fizz» если на 3, «Buzz» если на 5, иначе само число.",hints:["Сначала проверяй 15, потом 3, потом 5, иначе — число.","if i % 15 == 0: ... elif i % 3 == 0: ... elif i % 5 == 0: ... else: print(i).","Так как 15=3*5, порядок важен — 15 в первую очередь."],starterBlocks:e,starterPython:`for i in range(1, 16):
    if i % 15 == 0:
        ...
        print(i)
`,check:{kind:"output-match",expected:["1","2","Fizz","4","Buzz","Fizz","7","8","Fizz","Buzz","11","Fizz","13","14","FizzBuzz"]},reward:{coins:30,xp:40}}],u=[{id:"function-1",trainerId:"function",n:1,title:"Первая функция",prompt:"Определи функцию hello() которая печатает «Привет!», и вызови её один раз.",hints:["def hello(): ...",'В теле — print("Привет!").',"Не забудь вызвать: hello()."],starterBlocks:e,starterPython:`def hello():
    print("Привет!")
...
countdown(3)
`,check:{kind:"output-match",expected:["3","2","1","Бум!"]},reward:{coins:35,xp:45}}],f=[{id:"detective-1",trainerId:"detective",n:1,title:"Список подозреваемых",prompt:`ДАНО:
...
Детектив вызывает полицию. Дело закрыто!`}}],m=[{id:"path",emoji:"🐧",title:"Маршрут",tagline:"Двигай пингвина из старта в цель",description:"Учимся давать команды: идти вперёд, поворачивать налево и направо. 10 коротких задач от одного шага до сложного лабиринта.",color:"#6B5CE7",puzzles:l},{id:"tower",emoji:"🧱",title:"Башня",tagline:"Построй фигуру из блоков",description:"Цветные блоки, координаты X-Y-Z и циклы. Соберём квадрат 3×3, лестницу и целую пирамиду.",color:"#FF9454",puzzles:p},{id:"loop",emoji:"🔁",title:"Цикл",tagline:"Повторяй действия умно",description:"Один раз — медленно, а 5 раз — один цикл. Напечатаем 5 приветов, посчитаем сумму и построим ёлочку.",color:"#9FE8C7",puzzles:d},{id:"if",emoji:"🤔",title:"Условие",tagline:"Решай по ситуации",description:"if/else, elif и сравнения. От простого «больше-меньше» до полного FizzBuzz.",color:"#FFD43C",puzzles:h},{id:"function",emoji:"📦",title:"Функция",tagline:"Соберай своё: def и вызов",description:"Функции дают имена действиям. Напишем square(x), add(a,b) и даже рекурсивный countdown.",color:"#FFB4C8",puzzles:u},{id:"detective",emoji:"🕵️",title:"Детектив",tagline:"Раскрой кражу кода — списки, словари, функции",description:"Детектив Алекс расследует кражу исходного кода. 5 подозреваемых, 10 задач на Python: списки, словари, срезы строк, функции и логика and/or/not. Для тех, кто уже знает основы.",color:"#A06BE0",puzzles:f}];function k(r){return m.find(n=>n.id===r)}function x(r,n){const t=k(r);if(t)return t.puzzles.find(o=>o.n===n)}var s="ek_solved_";function a(r){try{const n=localStorage.getItem(s+r);if(!n)return new Set;const t=JSON.parse(n);return Array.isArray(t)?new Set(t.filter(o=>typeof o=="number")):new Set}catch{return new Set}}function _(r,n){const t=a(r);t.add(n);try{localStorage.setItem(s+r,JSON.stringify([...t]))}catch{}}function w(r){return a(r).size}function g(r){const n=a(r);for(let t=1;t<=10;t++)if(!n.has(t))return t;return 1}export{a,w as i,g as n,k as o,x as r,_ as s,m as t};
